# Fear spreads to nearby players
execute as @a[gamemode=!creative,gamemode=!spectator] at @s run execute as @a[distance=..8,gamemode=!creative,gamemode=!spectator] if score @s foggy.attention < @p foggy.attention run scoreboard players add @s foggy.attention 2
